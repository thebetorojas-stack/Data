...
detects three JPM file types and merges them:
  ...
  3. SNAPSHOT        — flat CSV, one row per entity, single trade date.
                       Organised as stacked BLOCKS separated by label rows:
                       "By Region", "By Country", "By Credit Bucket",
                       "By Sub Credit Bucket", "By Sov/Quasi Sov/Corp".
                       The loader is block-aware, so credit buckets and the
                       Sovereign/Quasi split are never mistaken for countries.

                       Analytics column names are resolved through an ALIAS
                       TABLE (SNAPSHOT_FIELD_ALIASES), case- and whitespace-
                       insensitive. J.P. Morgan renames these columns between
                       export vintages — as of the Aug-2026 export they ship
                       duration-weighted:

                         Yield to Worst     →  Dur Wgt Yield Wrst
                         STW (Trsy)         →  Dur Wgt STW (Trsy)
                         Z Spread to Worst  →  Dur Wgt Z- Spread to Wrst
                         Avr. Life          →  Weighted Avg Avg Life Wrst

                       Both old and new spellings are accepted. If a required
                       column resolves to nothing, the run prints a loud
                       warning and records it on the Diagnostics tab. It does
                       NOT fail silently — silent blanking of every spread and
                       yield cell was the original defect this build fixes.
...
----------------------------------
  •  snapshots_archive/ is append-only. Nothing is deleted or overwritten.
     A re-download that differs from an already-archived file for the same
     date is kept alongside it as snapshot_<date>__rev2.csv; the merge step
     then picks the more complete of the two, so a corrected download can
     repair a bad day without risking what is already on disk.
  •  The workbook is built to a temp file and swapped into place atomically.
     A crash mid-build leaves the previous workbook intact. If the target is
     locked (open in Excel), the run is saved under a timestamped name
...
COUNTRY_REGION: Dict[str, str] = {
    ...
    "Cote d Ivoire": "Africa", "Egypt": "Africa",
...
# ---------------------------------------------------------------------------
# SNAPSHOT COLUMN ALIASES  —  the fix for the "missing data" problem.
#
# J.P. Morgan changed the snapshot export header. The analytics columns are now
# shipped duration-weighted and renamed:
#
#     old header              →  current header (Aug-2026 export)
#     ----------------------     -------------------------------------
#     Yield to Worst          →  Dur Wgt Yield Wrst
#     STW (Trsy)              →  Dur Wgt STW (Trsy)
#     Z Spread to Worst       →  Dur Wgt Z- Spread to Wrst
#     Avr. Life               →  Weighted Avg Avg Life Wrst
#
# The old code looked those columns up by exact name, found nothing, and
# silently wrote blanks — which is why every spread/yield cell on the newest
# dates was empty while the total-return column kept working ("Index Level"
# never got renamed). Rather than hard-code the new names (JPM will rename
# them again), every field is resolved through an alias list at load time and
# the row is rewritten with canonical keys, so the ~3,000 lines of downstream
# code that ask for `snap["Yield to Worst"]` keep working untouched.
#
# Matching is case-insensitive and whitespace-collapsed, which also handles the
# trailing spaces JPM leaves on the return-decomposition headers
# (e.g. 'UST Return Daily Change (%) ').
# ---------------------------------------------------------------------------
SNAPSHOT_FIELD_ALIASES: Dict[str, List[str]] = {
    # identity / meta
    "Bam Id":              ["bam id", "bamid", "bam_id"],
    "Date":                ["date", "trade date", "as of date", "as-of date"],
    "Instrument":          ["instrument", "index name", "name"],
    # size / weight
    "Index Level":         ["index level", "cum tot ret idx", "total return index"],
    "Mkt Cap %":           ["mkt cap %", "mkt cap%", "market cap %", "index weight (%)",
                            "index weight %"],
    "Mkt Cap":             ["mkt cap", "market cap"],
    "Face OS":             ["face os", "face outstanding"],
    "Original Face OS":    ["original face os"],
    # analytics
    "Yield to Worst":      ["dur wgt yield wrst", "dur wgt yield worst",
                            "dur wgt yld wrst", "dur wgt yield to worst",
                            "yield to worst", "yld to worst", "ytw"],
    "STW (Trsy)":          ["dur wgt stw (trsy)", "dur wgt stw(trsy)",
                            "dur wgt stw trsy", "dur wgt stw",
                            "stw (trsy)", "stw(trsy)", "stw trsy", "stw",
                            "spread to worst (trsy)", "spread to worst"],
    "Z Spread to Worst":   ["dur wgt z- spread to wrst", "dur wgt z-spread to wrst",
                            "dur wgt z spread to wrst", "dur wgt z- spread to worst",
                            "z spread to worst", "z spread to wrst", "z-spread to worst"],
    "Spread Duration":     ["spread duration", "spread dur", "dur wgt spread duration"],
    "IR Duration":         ["ir duration to worst", "ir duration", "duration to worst",
                            "mod duration", "modified duration"],
    "IR Convexity":        ["ir convexity to worst", "ir convexity", "convexity to worst"],
    "Avr. Life":           ["weighted avg avg life wrst", "weighted avg average life wrst",
                            "weighted avg avg life worst", "avr. life", "avr life",
                            "avg life", "average life", "weighted avg life"],
    "Coupon Amt":          ["coupon amt", "coupon amount", "coupon"],
    # headline returns
    "Daily Change (%)":    ["daily change (%)", "daily change %", "daily change"],
    "MTD Change (%)":      ["mtd change (%)", "mtd change %", "mtd change"],
    "YTD Change (%)":      ["ytd change (%)", "ytd change %", "ytd change"],
    # counts
    "No. of Issues":       ["no. of issues", "no of issues", "number of issues", "# issues"],
    "No. of Issuer":       ["no. of issuer", "no of issuer", "no. of issuers",
                            "no of issuers", "number of issuers", "# issuers"],
    # ratings
    "Average S&P Rating":  ["average s&p rating", "avg s&p rating", "s&p rating",
                            "average sandp rating"],
    "Average Moody Rating": ["average moody rating", "average moody's rating",
                             "average moodys rating", "avg moody rating", "moody rating"],
    "Average Fitch Rating": ["average fitch rating", "avg fitch rating", "fitch rating"],
    # return decomposition (new in the Aug-2026 export — previously unused)
    "UST Return Daily (%)":    ["ust return daily change (%)", "ust return daily change"],
    "UST Return MTD (%)":      ["ust return mtd change (%)", "ust return mtd change"],
    "UST Return YTD (%)":      ["ust return ytd change (%)", "ust return ytd change"],
    "Spread Return Daily (%)": ["spread return daily change (%)", "spread return daily change"],
    "Spread Return MTD (%)":   ["spread return mtd change (%)", "spread return mtd change"],
    "Spread Return YTD (%)":   ["spread return ytd change (%)", "spread return ytd change"],
    "Price Return Daily (%)":  ["price return daily change (%)", "price return daily change"],
    "Price Return MTD (%)":    ["price return mtd change (%)", "price return mtd change"],
    "Price Return YTD (%)":    ["price return ytd change (%)", "price return ytd change"],
    "Coupon Return Daily (%)": ["coupon return daily change (%)", "coupon return daily change"],
    "Coupon Return MTD (%)":   ["coupon return mtd change (%)", "coupon return mtd change"],
    "Coupon Return YTD (%)":   ["coupon return ytd change (%)", "coupon return ytd change"],
}

# Canonical fields the workbook genuinely depends on. If any of these fail to
# resolve against the header of a snapshot file, the run prints a loud warning
# and records it on the Diagnostics tab — the old script failed silently.
SNAPSHOT_REQUIRED_FIELDS: List[str] = [
    "Instrument", "Date", "Index Level", "Mkt Cap %",
    "Yield to Worst", "STW (Trsy)", "Spread Duration",
    "Average S&P Rating", "MTD Change (%)", "YTD Change (%)",
]

_SNAPSHOT_ALIAS_LOOKUP: Dict[str, str] = {}
for _canon, _spellings in SNAPSHOT_FIELD_ALIASES.items():
    _SNAPSHOT_ALIAS_LOOKUP[re.sub(r"\s+", " ", _canon.strip().lower())] = _canon
    for _sp in _spellings:
        _SNAPSHOT_ALIAS_LOOKUP[re.sub(r"\s+", " ", _sp.strip().lower())] = _canon


def _header_key(h: str) -> str:
    """Normalize a CSV header for alias matching (case, spacing, stray quotes)."""
    return re.sub(r"\s+", " ", (h or "").replace("\ufeff", "").strip().strip('"').strip().lower())


# --- Snapshot section handling ------------------------------------------------
# The snapshot CSV is not a flat list of countries. It is a set of stacked
# blocks separated by label rows ("By Region", "By Country", "By Credit Bucket",
# "By Sub Credit Bucket", "By Sov/Quasi Sov/Corp"). The old loader skipped only
# the label rows, so every credit bucket and the Sovereign/Quasi split were
# ingested AS COUNTRIES. Consequences: ~13 phantom "countries" in the entity
# universe, and — worse — their 'Mkt Cap %' values were merged into the country
# weight series, adding roughly 300% of phantom index weight on every snapshot
# date and corrupting the Rating_Trend IG / HY / NR shares.
SECTION_LABELS: Dict[str, str] = {
    "by region":                "region",
    "by country":               "country",
    "by credit bucket":         "credit",
    "by sub credit bucket":     "credit",
    "by subcredit bucket":      "credit",
    "by rating":                "credit",
    "by sov/quasi sov/corp":    "sovquasi",
    "by sov/quasi":             "sovquasi",
    "by sector":                "sector",
    "by maturity":              "maturity",
    "by maturity bucket":       "maturity",
}

# Bam Id is the authoritative section signal when present — it survives any
# relabelling of the separator rows.
_BAMID_REGION = {"AFRICA", "ASIA", "EUROPE", "LATIN", "MIDEAST", "NONLAT",
                 "MIDDLEEAST", "NONLATIN"}
_BAMID_SOVQUASI = {"QUASI", "SOV", "SOVEREIGN", "CORP"}


def _section_from_bamid(bam: str) -> Optional[str]:
    b = (bam or "").strip().upper()
    if not b:
        return None
    if b in ("EMBIG", "EMBIGD", "EMBI", "EMBIGLOBAL"):
        return "index"
    if not b.startswith("EMBIG"):
        return None
    suffix = b.split("_", 1)[1] if "_" in b else ""
    if not suffix:
        return None
    if suffix in _BAMID_REGION:
        return "region"
    if suffix.startswith("CREDIT_") or suffix in ("NONIG", "IG"):
        return "credit"
    if suffix in _BAMID_SOVQUASI:
        return "sovquasi"
    return "country"


# Snapshot credit-bucket labels → the RATING_ORDER keys the workbook already
# uses (which come from the returns file's "Credit XX only" column naming).
# Mapping them through means the By_Rating tab now refreshes from a snapshot
# instead of going stale whenever there is no returns file for that date.
SNAPSHOT_CREDIT_TO_RATING: Dict[str, str] = {
    "investment grade":      "Credit IG only",
    "ig":                    "Credit IG only",
    "non investment grade":  "Credit Non-IG",
    "non-investment grade":  "Credit Non-IG",
    "noninvestment grade":   "Credit Non-IG",
    "high yield":            "Credit Non-IG",
    "aa":                    "Credit AA only",
    "a":                     "Credit A only",
    "bbb":                   "Credit BBB only",
    "bb":                    "Credit BB only",
...
def _canon_key(s: str) -> str:
    """Aggressive lookup key: case-folded, punctuation- and spacing-insensitive.

    'MONTENEGRO', 'Montenegro' and ' montenegro ' all collapse to 'montenegro';
    "Cote D'Ivoire" and 'Cote d Ivoire' both collapse to 'cote d ivoire';
    'Trinidad And Tobago' and 'Trinidad & Tobago' both collapse to
    'trinidad and tobago'. This is what lets the loader survive JPM shipping
    the same country under a different capitalisation from one file to the next
    — which it does (the Aug-2026 export ships MONTENEGRO, REPUBLIC OF CONGO
    and DEMOCRATIC REPUBLIC OF CONGO in caps while everything else is title
    case). Under the old exact-match logic those three rows were silently
    dropped from the entire workbook.
    """
...
# Canonical-name lookup table, built once from the taxonomy + the alias map.
_CANONICAL_BY_KEY: Dict[str, str] = {}
for _n in COUNTRY_REGION:
    _CANONICAL_BY_KEY.setdefault(_canon_key(_n), _n)
for _alias, _target in NAME_ALIASES.items():
    _CANONICAL_BY_KEY[_canon_key(_alias)] = _target


def normalize_name(name: str) -> str:
    raw = (name or "").strip().replace("&amp;", "&").replace("&#38;", "&")
    if not raw:
        return raw
    if raw in NAME_ALIASES:
        return NAME_ALIASES[raw]
    if raw in COUNTRY_REGION:
        return raw
    return _CANONICAL_BY_KEY.get(_canon_key(raw), raw)
...
def resolve_snapshot_columns(fieldnames: Iterable[str]) -> Tuple[Dict[str, str], List[str], List[str]]:
    """Map canonical field names onto the actual headers of one snapshot file.

    Returns (canonical -> actual_header, unresolved_required, unrecognised_headers).
    """
    ...
    for h in fieldnames or []:
        if h is None:
            continue
        canon = _SNAPSHOT_ALIAS_LOOKUP.get(_header_key(h))
        if canon is None:
            if _header_key(h):
                unrecognised.append(h.strip())
            continue
        # First header wins — protects against duplicate/legacy columns that
        # JPM sometimes leaves in the export alongside the new ones.
        mapping.setdefault(canon, h)
    unresolved = [c for c in SNAPSHOT_REQUIRED_FIELDS if c not in mapping]
    return mapping, unresolved, unrecognised


def _snapshot_entity_key(instrument: str, bam: str, section: str) -> Optional[str]:
    """Turn one snapshot row into a namespaced entity key, or None to drop it.

    Namespaces keep the different block types apart so that credit buckets and
    the Sovereign/Quasi split can never again be mistaken for countries.
    """
    name = (instrument or "").strip()
    if not name:
        return None
    low = _header_key(name)

    if low in ("embi global", "embi global diversified", "embi global core",
               "embi global div", "embigd"):
        # Either flavour of the index unifies under the same internal key.
        return "INDEX:EMBI"
    ...
    if section == "region":
        canon = SNAPSHOT_REGION_LABEL.get(name)
        if canon:
            return f"REGION:{canon}"
        if low in ("non latin", "nonlatin", "non-latin"):
            return "AGG:NonLatin"
        for label, canon in SNAPSHOT_REGION_LABEL.items():
            if _header_key(label) == low:
                return f"REGION:{canon}"
        return f"AGG:{name}"

    if section == "credit":
        return SNAPSHOT_CREDIT_TO_RATING.get(low, f"CREDIT:{name}")

    if section == "sovquasi":
        return f"SOVQUASI:{name}"

    if section in ("sector", "maturity", "other"):
        return f"OTHER:{name}"

    # section == 'country' (or unknown, which we treat as country)
...
def load_snapshot(path: Path) -> Tuple[Optional[datetime], Dict[str, Dict[str, Any]]]:
    ...
    Every row is rewritten with CANONICAL field names (see
    SNAPSHOT_FIELD_ALIASES) so downstream code is insulated from JPM header
    churn, and every row is tagged with the block it came from so credit
    buckets / Sov-Quasi rows never leak into the country universe.
    """
    with _open_csv(path) as f:
    ...
    colmap, unresolved, unrecognised = resolve_snapshot_columns(fieldnames)
    SNAPSHOT_LOAD_REPORT.append({
        "file": path.name,
        "resolved": dict(colmap),
        "unresolved_required": list(unresolved),
        "unrecognised_headers": list(unrecognised),
    })

    def cell(row: Dict[str, Any], canon: str) -> str:
    ...
    section = "index"          # the first data row is the index total
    seen_any_data_row = False

    for row in rows:
        instrument = cell(row, "Instrument")
        if not instrument:
            continue
        low = _header_key(instrument)

        # Block separator rows: "By Region", "By Country", ...
        if low.startswith("by "):
            section = SECTION_LABELS.get(low, "other")
            continue

        bam = cell(row, "Bam Id")
        sec = _section_from_bamid(bam) or (section if seen_any_data_row else "index")
        seen_any_data_row = True

        if snap_date is None:
            try:
                snap_date = _parse_date(cell(row, "Date"))
            except ValueError:
                pass

        key = _snapshot_entity_key(instrument, bam, sec)
        if key is None:
            continue
        if ":" not in key and key not in RATING_ORDER and key not in COUNTRY_REGION:
            # A genuine constituent with no region mapping. Its data is still
            # loaded; the Diagnostics tab reports it so the taxonomy can be
            # extended instead of the row vanishing without trace.
            UNMAPPED_COUNTRIES.add(key)

        # Canonical view of the row, plus the raw row kept under "_raw" for
        # anything that wants a column we haven't aliased yet.
        canon_row: Dict[str, Any] = {c: cell(row, c) for c in colmap}
        canon_row["_section"] = sec
        canon_row["_bam_id"] = bam
        canon_row["_raw"] = row
        out[key] = canon_row
...
# Populated by load_snapshot; surfaced on the Diagnostics tab and on stderr so a
# future JPM header rename is caught immediately rather than silently blanking
# half the workbook (which is exactly what happened here).
SNAPSHOT_LOAD_REPORT: List[Dict[str, Any]] = []

# Constituents seen in a snapshot's "By Country" block that have no entry in
# COUNTRY_REGION. Reported, never dropped.
UNMAPPED_COUNTRIES: set = set()
...
def merge_snapshots(file_results: List[Tuple[Optional[datetime], Dict[str, Dict[str, Any]]]]
    ...
    def _completeness(s: Dict[str, Dict[str, Any]]) -> Tuple[int, int]:
        """(rows carrying a usable spread, total rows) — used to pick a winner
        when the same trade date arrives twice (e.g. 'JP latest.csv' and its
        archived copy, or a re-download that fixed a truncated export).
        Choosing the more complete file rather than whichever happened to load
        first means a re-download can repair a bad day's data; nothing is ever
        deleted either way, since both files stay in the archive."""
        filled = sum(
            1 for row in s.values()
            if isinstance(row, dict) and _parse_value(row.get("STW (Trsy)")) is not None
        )
        return filled, len(s)
    ...
    for d, s in file_results:
        if d is None:
            continue
        incumbent = by_date.get(d)
        if incumbent is None or _completeness(s) > _completeness(incumbent):
            by_date[d] = s
...
def snapshot_to_returns(
    ...
    for ent, data in snap_data.items():
        if not isinstance(ent, str) or not data:
            continue
        if ent in _SNAPSHOT_ENTITY_TO_RETURNS:
            # Index + region rollups → the names the returns file uses.
            returns_ent = _SNAPSHOT_ENTITY_TO_RETURNS[ent]
        elif ent in RATING_ORDER:
            # Credit buckets, already mapped to the returns-file rating keys.
            # Feeding these through is what keeps the By_Rating tab current on
            # snapshot-only refreshes.
            returns_ent = ent
        elif ":" in ent:
            # AGG: / SOVQUASI: / CREDIT: / OTHER: — deliberately kept out of the
            # entity time series so they can never be treated as countries.
            continue
        else:
            returns_ent = ent
        for field, metric in _SNAPSHOT_FIELD_TO_METRIC.items():
            v = _parse_value(data.get(field))
            if v is None:
                continue
...
def archive_snapshots(input_paths: List[Path], project_dir: Path) -> Tuple[Path, List[Tuple[str, str]]]:
    ...
    for path in input_paths:
        ...
        if snap_date is None:
            continue
        stamp = snap_date.strftime("%Y-%m-%d")
        archive_name = f"snapshot_{stamp}.csv"
        archive_path = archive_dir / archive_name

        if archive_path.exists():
            # Same date already archived. If the incoming file is byte-identical
            # there is nothing to do. If it differs — a corrected or extended
            # re-download — keep BOTH under a revision name. Archived files are
            # never overwritten and never deleted; the merge step picks the more
            # complete one per date, so a repaired download can fix a bad day
            # without any risk to the history already on disk.
            try:
                same = (archive_path.stat().st_size == path.stat().st_size
                        and archive_path.read_bytes() == path.read_bytes())
            except OSError:
                same = True
            if same:
                continue
            rev = 2
            while (archive_dir / f"snapshot_{stamp}__rev{rev}.csv").exists():
                rev += 1
            archive_name = f"snapshot_{stamp}__rev{rev}.csv"
            archive_path = archive_dir / archive_name

        try:
            shutil.copy2(path, archive_path)
            newly_archived.append((stamp, archive_name))
        except (OSError, PermissionError) as exc:
...
class Builder:
    ...
    def _merge_snapshot_weights(
        ...
        for snap_date, snap_data in snap_history:
            for ent, data in snap_data.items():
                # COUNTRY WEIGHTS ONLY. Anything namespaced (REGION:/INDEX:/AGG:/
                # CREDIT:/SOVQUASI:/OTHER:) or mapped to a rating bucket is a
                # rollup, not a constituent. Letting those through was adding
                # ~300% of phantom index weight on every snapshot date
                # (IG+Non-IG = 100, sub-buckets = 100, Sov+Quasi = 100) and
                # corrupting the Rating_Trend IG / HY / NR shares.
                if not isinstance(ent, str) or ":" in ent or ent in RATING_ORDER:
                    continue
                # A country not yet in COUNTRY_REGION is still a real
                # constituent — keep its weight (it belongs in the index
                # totals) and let the Diagnostics tab flag the missing region
                # mapping rather than silently discarding it.
                w = _parse_value(data.get("Mkt Cap %"))
                if w is None:
                    continue
    ...
    def build_instructions(self):
        ...
        sections = [
            ...
            ("Snapshot spreads/yields/TR also feed the time series",
             ...
             "JPM renames these source columns between export vintages — as of the Aug-2026 export "
             "they arrive duration-weighted as 'Dur Wgt Yield Wrst', 'Dur Wgt STW (Trsy)' and "
             "'Weighted Avg Avg Life Wrst'. The loader resolves both old and new spellings through "
             "an alias table, so you do not need to rename anything. If JPM invents a spelling the "
             "table does not know, the run warns on screen and the DIAGNOSTICS tab names the exact "
             "column — it will not quietly write blanks. "
            ...
            ("Check the Diagnostics tab if anything looks empty",
             "The last tab in the workbook reports which files loaded, how many entities carry a "
             "spread / yield / total return on the latest date, exactly which CSV header each "
             "analytics field resolved to, and any constituent that arrived without a region "
             "mapping. It is the fastest way to tell a genuine index change (a country left the "
             "index) from a broken parse (JPM renamed a column)."),
            ("",""),
    ...
    def build_snapshot(self):
        ...
        cols = [
            ...
            ("Z-Spread", 11, "Z Spread to Worst"),
            ("Spread Dur", 11, "Spread Duration"),
            ("IR Dur", 10, "IR Duration"),
    ...
    def _fill_snapshot_row(self, ws, row, snap, cols, *, label_style):
        for i, (h, _, key) in enumerate(cols, start=1):
            ...
            try:
                ...
                elif key in ("STW (Trsy)", "Z Spread to Worst"):
                    # Spread to worst is quoted in bps in JPM's snapshot dump
                    cell.number_format = "0"
                elif key in ("Yield to Worst", "Spread Duration",
                             "IR Duration", "Avr. Life"):
                    cell.number_format = "0.00"
    ...
    def _ratings_as_of(self, target_date: datetime) -> Dict[str, int]:
        ...
        for ent, data in chosen.items():
            # Countries only — every rollup key is namespaced with a colon, and
            # credit buckets resolve to RATING_ORDER names.
            if not isinstance(ent, str) or ":" in ent or ent in RATING_ORDER:
                continue
    ...
    def build_return_decomp(self):
        """Rate vs. credit split of index returns, straight from the snapshot.

        The JPM export carries UST / Spread / Price / Coupon return legs at
        daily, MTD and YTD frequency. The previous build ignored all twelve
        columns. They are the cleanest available answer to the question of how
        much of EMBIG performance is the Treasury leg rather than the credit
        leg, so they get their own tab: identity checks included, because
        UST + Spread should reconcile to the headline total and any drift is a
        signal the export is stale or mixed-vintage.
        """
        ws = self.wb.create_sheet("Return_Decomp")
        ws.sheet_view.showGridLines = False
        ws["A1"] = "Return decomposition — rate leg vs. credit leg"
        self._font(ws["A1"], size=14, bold=True, color=COLOR_HEADER_BG)
        ws.row_dimensions[1].height = 22

        if not self.snap_date:
            ws["A2"] = "No snapshot loaded — this tab is snapshot-sourced."
            self._font(ws["A2"], color=COLOR_NOTE, italic=True)
            return

        ws["A2"] = (f"Snapshot {self.snap_date:%Y-%m-%d}. UST leg = return from the Treasury "
                    f"curve; Spread leg = return from spread moves. Total ≈ UST + Spread "
                    f"(residual in the Check columns). All figures %, as reported by JPM.")
        ...
        cols = [
            ("Bucket", 16, None),
            ("Entity", 26, None),
            ("MTD tot", 10, "MTD Change (%)"),
            ("MTD UST", 10, "UST Return MTD (%)"),
            ("MTD sprd", 10, "Spread Return MTD (%)"),
            ("MTD chk", 9, None),
            ("YTD tot", 10, "YTD Change (%)"),
            ("YTD UST", 10, "UST Return YTD (%)"),
            ("YTD sprd", 10, "Spread Return YTD (%)"),
            ("YTD cpn", 10, "Coupon Return YTD (%)"),
            ("YTD px", 10, "Price Return YTD (%)"),
            ("YTD chk", 9, None),
            ("Sprd dur", 9, "Spread Duration"),
            ("IR dur", 9, "IR Duration"),
        ]
        header_row = 4
        for i, (h, w, _) in enumerate(cols, start=1):
            self._hdr(ws.cell(row=header_row, column=i), h)
        ...
        def _emit(row: int, bucket: str, label: str, snap: Dict[str, Any], style: str) -> bool:
            if not snap:
                return False
            vals = {key: _parse_value(snap.get(key)) for (_, _, key) in cols if key}
            if all(v is None for v in vals.values()):
                return False
            if style == "index":
                self._index_lbl(ws.cell(row=row, column=1), bucket)
                self._index_lbl(ws.cell(row=row, column=2), label)
            elif style == "region":
                self._region_lbl(ws.cell(row=row, column=1), bucket)
                self._region_lbl(ws.cell(row=row, column=2), label)
            elif style == "rating":
                self._rating_lbl(ws.cell(row=row, column=1), bucket)
                self._rating_lbl(ws.cell(row=row, column=2), label)
            else:
                for c, txt in ((1, bucket), (2, label)):
                    cell = ws.cell(row=row, column=c, value=txt)
                    cell.border = THIN_BORDER
                    self._font(cell)
            for i, (_, _, key) in enumerate(cols, start=1):
                if key is None:
                    continue
                v = vals.get(key)
                fmt = "0.00" if key in ("Spread Duration", "IR Duration") else "+0.00;-0.00;0.00"
                self._val(ws.cell(row=row, column=i), v if v is not None else "",
                          fmt=fmt, color=COLOR_HARDCODE)
            # Residual checks: total less the two legs.
            for chk_col, tot, ust, spr in ((6, "C", "D", "E"), (12, "G", "H", "I")):
                cell = ws.cell(row=row, column=chk_col)
                cell.value = f'=IFERROR({tot}{row}-{ust}{row}-{spr}{row}, "")'
                cell.number_format = "+0.00;-0.00;0.00"
                cell.border = THIN_BORDER
                self._font(cell, color=COLOR_FORMULA)
            return True

        row = header_row + 1
        if _emit(row, "Index", INDEX_NAME, self.snap_data.get("INDEX:EMBI") or {}, "index"):
            row += 1
        row += 1

        for region in REGION_ORDER:
            snap = self.snap_data.get(f"REGION:{region}") or {}
            if _emit(row, "Region", REGION_AGGREGATE[region], snap, "region"):
                row += 1
        nonlat = self.snap_data.get("AGG:NonLatin") or {}
        if _emit(row, "Region", "Non-Latin", nonlat, "region"):
            row += 1
        row += 1

        for rating in RATING_ORDER:
            snap = self.snap_data.get(rating) or {}
            if _emit(row, "Credit", RATING_DISPLAY.get(rating, rating), snap, "rating"):
                row += 1
        for key in sorted(k for k in self.snap_data if k.startswith("SOVQUASI:")):
            if _emit(row, "Sov/Quasi", key.split(":", 1)[1], self.snap_data[key], "rating"):
                row += 1
        row += 1

        for region in REGION_ORDER:
            for c in self.countries_by_region.get(region, []):
                if _emit(row, region, c, self.snap_data.get(c) or {}, "country"):
                    row += 1
        last_row = row - 1
        ...
        ws.cell(row=note, column=1,
                value="Check columns should sit near zero. A persistent non-zero residual means the "
                      "legs and the headline total came from different vintages of the export — "
                      "re-download rather than trusting the split.")
        ws.merge_cells(start_row=note, start_column=1, end_row=note, end_column=14)
        self._font(ws.cell(row=note, column=1), color=COLOR_NOTE, italic=True)

    # --------- Diagnostics ----------
    def build_diagnostics(self):
        """Loud, visible record of what the loader did and did not understand.

        The original failure was silent: JPM renamed four analytics columns, the
        loader matched none of them, and the workbook was written with empty
        spread and yield cells and no warning anywhere. This tab exists so that
        never happens quietly again.
        """
        ...
        ws["B1"] = "Loader diagnostics"
        self._font(ws["B1"], size=14, bold=True, color=COLOR_HEADER_BG)
        ws["B2"] = f"Build run {datetime.now():%Y-%m-%d %H:%M}. Check this tab first if a column looks empty."
        self._font(ws["B2"], color=COLOR_NOTE, italic=True)
        ...
        def _section(txt: str) -> None:
            ...
            self._font(ws.cell(row=row, column=2), bold=True, color=COLOR_HEADER_BG)
            row += 1

        def _kv(label: str, value: str, *, bad: bool = False) -> None:
            nonlocal row
            ws.cell(row=row, column=2, value=label)
            self._font(ws.cell(row=row, column=2))
            cell = ws.cell(row=row, column=3, value=value)
            self._font(cell, color="C00000" if bad else COLOR_HARDCODE, bold=bad)
            row += 1

        _section("FILES LOADED")
        for kind, name in self.sources:
            _kv(kind, name)
        if not self.sources:
            _kv("(none)", "")
        row += 1
        ...
        _section("COVERAGE")
        _kv("Dates in time series", str(len(self.dates)))
        if self.dates:
            _kv("First / last date", f"{self.dates[0]:%Y-%m-%d}  →  {self.dates[-1]:%Y-%m-%d}")
        _kv("Entity × metric series", str(len(self.series)))
        _kv("Snapshots merged", str(len(self.snap_history)))
        _kv("Weight history dates", str(len(self.weight_dates)))
        row += 1

        # Per-metric fill on the most recent date — this is the check that would
        # have caught the original bug on day one.
        _section("LATEST-DATE FILL BY METRIC")
        if self.dates:
            last = len(self.dates) - 1
            for label, metric in (("Spread — STW (Trsy)", METRICS["spread"]),
                                  ("Yield — Yld to Maturity", METRICS["yield"]),
                                  ("Total return index", METRICS["tret"])):
                filled = sum(
                    1 for (ent, m), vals in self.series.items()
                    if m == metric and last < len(vals) and vals[last] is not None
                )
                total = sum(1 for (ent, m) in self.series if m == metric)
                _kv(label, f"{filled} of {total} entities populated",
                    bad=(total > 0 and filled == 0))
        row += 1

        _section("SNAPSHOT COLUMN RESOLUTION")
        if not SNAPSHOT_LOAD_REPORT:
            _kv("(no snapshot files loaded)", "")
        for rep in SNAPSHOT_LOAD_REPORT:
            unresolved = rep["unresolved_required"]
            _kv(rep["file"],
                "all required columns resolved" if not unresolved
                else f"MISSING: {', '.join(unresolved)}",
                bad=bool(unresolved))
            if rep["unrecognised_headers"]:
                _kv("   headers not recognised",
                    ", ".join(rep["unrecognised_headers"][:8])
                    + ("  …" if len(rep["unrecognised_headers"]) > 8 else ""))
        row += 1

        _section("KEY FIELD MAPPING (latest snapshot file)")
        if SNAPSHOT_LOAD_REPORT:
            resolved = SNAPSHOT_LOAD_REPORT[-1]["resolved"]
            for canon in ("Index Level", "Yield to Worst", "STW (Trsy)",
                          "Z Spread to Worst", "Spread Duration", "IR Duration",
                          "Avr. Life", "Mkt Cap %"):
                _kv(canon, resolved.get(canon, "— NOT FOUND —"),
                    bad=canon not in resolved)
        row += 1

        _section("CONSTITUENTS WITH NO REGION MAPPING")
        if UNMAPPED_COUNTRIES:
            for c in sorted(UNMAPPED_COUNTRIES):
                _kv(c, "add to COUNTRY_REGION — data loaded but not shown by region",
                    bad=True)
        else:
            _kv("(none)", "every constituent mapped to a region")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    # archive_snapshots reads each snapshot to date-stamp it, which populates
    # the load report. Reset it so the report reflects the real load pass only
    # and each file appears exactly once.
    SNAPSHOT_LOAD_REPORT.clear()
    UNMAPPED_COUNTRIES.clear()
    if newly_archived:
    ...
    # ---- Loud reporting: the original bug was that this was all silent ----
    problems = [r for r in SNAPSHOT_LOAD_REPORT if r["unresolved_required"]]
    if problems:
        print("\n  *** SNAPSHOT COLUMN MISMATCH ***", file=sys.stderr)
        for r in problems:
            print(f"    {r['file']}: could not resolve "
                  f"{', '.join(r['unresolved_required'])}", file=sys.stderr)
            if r["unrecognised_headers"]:
                print(f"      headers present but unrecognised: "
                      f"{', '.join(r['unrecognised_headers'][:10])}", file=sys.stderr)
        print("    Add the new spelling to SNAPSHOT_FIELD_ALIASES. "
              "Affected cells will be blank until you do.\n", file=sys.stderr)
    if UNMAPPED_COUNTRIES:
        print(f"\n  NOTE: constituents with no region mapping (data loaded, but "
              f"not shown on region-grouped tabs): {', '.join(sorted(UNMAPPED_COUNTRIES))}\n"
              f"        Add them to COUNTRY_REGION.", file=sys.stderr)

    if dates:
        last = len(dates) - 1
        for label, metric in (("spread", METRICS["spread"]), ("yield", METRICS["yield"]),
                              ("total return", METRICS["tret"])):
            filled = sum(1 for (e, m), v in series.items()
                         if m == metric and last < len(v) and v[last] is not None)
            if filled == 0:
                print(f"  WARNING: no {label} data on the latest date "
                      f"({dates[last]:%Y-%m-%d}) — see the Diagnostics tab.",
                      file=sys.stderr)
    ...
    # Atomic write: build to a temp file alongside the target, then swap. If the
    # save throws halfway through, the previous workbook is still intact rather
    # than left truncated. Never write directly over the deliverable.
    ...
    try:
        ...
        # Typically the target is open in Excel and locked. Keep the new file
        # under a fallback name rather than losing the run's work.
        fallback = out_path.with_name(
            f"{out_path.stem}_{datetime.now():%Y%m%d_%H%M%S}{out_path.suffix}")
        tmp_path.replace(fallback)
        print(f"\n  Could not replace {out_path.name} ({exc}).\n"
              f"  Wrote {fallback} instead — close the open workbook and rename.",
              file=sys.stderr)
        return 0
    print(f"\n  Wrote {out_path}")
    if archive_dir is not None:
        n_archived = len(list(archive_dir.glob('*.csv'))) if archive_dir.exists() else 0
        print(f"  Snapshot archive: {n_archived} file(s) in {archive_dir} "
              f"(append-only — nothing is ever deleted or overwritten)")
    return 0
