...
# -*- coding: utf-8 -*-
"""
Argentina CPI seasonality pack - self-contained builder.

Pulls INDEC's national CPI (headline, core/nucleo, regulated, seasonal) from the
official public series API and writes a formatted Excel workbook: seasonality
charts by calendar year, a year x month heatmap, momentum, component split,
scale-free seasonal factors and a bull/base/bear forecast to Dec-2027.

    python argentina_cpi.py                     build with the latest data
    python argentina_cpi.py --offline           use the history embedded below
    python argentina_cpi.py --out "C:\\temp\\cpi.xlsx"
    python argentina_cpi.py --no-verify         corporate TLS interception
    python argentina_cpi.py --csv               also write a tidy CSV
    python argentina_cpi.py --proxy http://proxy.corp:8080

Requires Python 3.7+ and openpyxl (pip install openpyxl). Nothing else.
If the network is blocked it falls back to the embedded history automatically,
so the script always produces a workbook.

Source: INDEC, Indice de precios al consumidor nacional, base dic-2016 = 100,
via apis.datos.gob.ar. Series ids are in SERIES below.
"""
...
SERIES = {
    "head": "148.3_INIVELNAL_DICI_M_26",   # IPC nivel general nacional
    "core": "148.3_INUCLEONAL_DICI_M_19",  # IPC nucleo nacional
    "reg":  "148.3_IREGULANAL_DICI_M_22",  # IPC regulados nacional
    "est":  "148.3_IESTACINAL_DICI_M_25",  # IPC estacionales nacional
}
# Published INDEC Dec/Dec prints, used as a correctness gate on whatever we fetch.
CHECKS = {2019: 53.8, 2020: 36.1, 2021: 50.9, 2022: 94.8,
          2023: 211.4, 2024: 117.8, 2025: 31.5}


def say(msg):
    """Print without blowing up on a cp1252 Windows console."""
    try:
        print(msg)
...
EMBEDDED["head"] = """2018-12,184.2552
2019-01,189.6101
2019-02,196.7501
2019-03,205.9571
2019-04,212.9596414
...
EMBEDDED["core"] = """2018-12,178.9326
2019-01,184.3056
2019-02,191.4115
2019-03,200.1771
2019-04,207.7179
...
def parse_pairs(text):
    """'YYYY-MM,value' lines -> {'YYYY-MM': float}."""
    out = {}
...
def fetch_series(series_id, start="2018-12-01", verify=True, timeout=45):
    """Pull one series as CSV from the INDEC/datos.gob.ar API."""
    url = API + "?" + urlencode({"ids": series_id, "format": "csv",
                                 "start_date": start, "limit": "5000"})
    ctx = None
    if not verify:
        ctx = ssl.create_default_context()
        ctx.check_hostname = False
        ctx.verify_mode = ssl.CERT_NONE
    req = Request(url, headers={"User-Agent": "Mozilla/5.0 (compatible; cpi-pack/1.0)"})
    ...
    for row in rows[1:]:
        ...
        try:
            ...
            continue
    if len(out) < 12:
        raise ValueError("series %s returned %d rows" % (series_id, len(out)))
    return out


def dec_dec(series, year):
    a, b = series.get("%d-12" % year), series.get("%d-12" % (year - 1))
    if a is None or b is None:
        return None
    return (a / b - 1) * 100


def validate(head, tol=0.15):
    """Compounded monthlies must reproduce INDEC's published Dec/Dec prints."""
    bad = []
    for year, official in CHECKS.items():
        got = dec_dec(head, year)
        if got is None:
            continue
        if abs(got - official) > tol:
            bad.append((year, official, got))
    return bad


def load_data(offline=False, verify=True, proxy=None):
    """Fetched values win; the embedded history backfills anything missing."""
    data = dict((k, parse_pairs(v)) for k, v in EMBEDDED.items())
    if offline:
        say("Offline mode: using the history embedded in this script.")
        return data, False
    ...
    for key, sid in SERIES.items():
        try:
            fetched = fetch_series(sid, verify=verify)
            data[key].update(fetched)
            live = True
        except (URLError, HTTPError, ValueError, OSError, ssl.SSLError) as exc:
            say("  ! could not fetch %-4s (%s) - using embedded history for it"
                % (key, type(exc).__name__))
    if not live:
        say("No series could be fetched. Check your proxy settings, or run with")
        say("--offline to silence this. Building from the embedded history instead.")
        say("Hints: set HTTPS_PROXY, or pass --proxy http://host:port, or --no-verify")
        say("       if your firewall re-signs TLS certificates.")
    return data, live


def write_csv(data, path):
    months = sorted(data["head"])
    with open(path, "w", newline="", encoding="utf-8") as fh:
        w = _csv.writer(fh)
        w.writerow(["month", "headline_index", "core_index", "regulated_index",
                    "seasonal_index", "headline_mom_pct", "core_mom_pct"])
        prev = None
        for m in months:
            row = [m] + [data[k].get(m, "") for k in ("head", "core", "reg", "est")]
            for k in ("head", "core"):
                cur, pre = data[k].get(m), data[k].get(prev) if prev else None
                row.append(round((cur / pre - 1) * 100, 4) if (cur and pre) else "")
            w.writerow(row)
            prev = m


# ---------------------------------------------------------------- workbook
def build(HEAD, CORE, REG, EST):
    ...
    END_Y = max(2027, LAST_Y + 1)
    GRID = []
    _y, _m = 2018, 12
    while (_y, _m) <= (END_Y, 12):
        GRID.append("%d-%02d" % (_y, _m))
        _m += 1
        if _m == 13:
            _y, _m = _y + 1, 1
...
def main(argv=None):
    ap = argparse.ArgumentParser(
        description="Build the Argentina INDEC CPI seasonality workbook.")
    ap.add_argument("--out", default="Argentina_inflation_seasonality.xlsx",
                    help="output .xlsx path")
    ap.add_argument("--offline", action="store_true",
                    help="skip the network, use the embedded history")
    ap.add_argument("--no-verify", action="store_true",
                    help="do not verify TLS certificates (corporate MITM proxies)")
    ap.add_argument("--proxy", default=None,
                    help="proxy URL, e.g. http://proxy.corp:8080 "
                         "(HTTPS_PROXY / HTTP_PROXY are picked up automatically)")
    ap.add_argument("--csv", action="store_true",
                    help="also write a tidy CSV next to the workbook")
    ap.add_argument("--force", action="store_true",
                    help="write the workbook even if the validation gate fails")
    args = ap.parse_args(argv)

    try:
        import openpyxl  # noqa: F401
    except ImportError:
        say("openpyxl is required.  pip install openpyxl")
        say("(no admin rights?  pip install --user openpyxl)")
        return 2

    say("Argentina CPI pack")
    data, live = load_data(offline=args.offline,
                           verify=not args.no_verify,
                           proxy=args.proxy)

    bad = validate(data["head"])
    if bad:
        say("VALIDATION FAILED - compounded monthlies do not match INDEC's prints:")
        for year, official, got in bad:
            say("   %d: published %.1f%%, computed %.2f%%" % (year, official, got))
        if not args.force:
            say("Refusing to write a workbook from data that does not tie out.")
            say("Re-run, or use --offline, or pass --force to override.")
            return 1
    else:
        say("Validation OK - Dec/Dec reproduces every published INDEC print.")

    last = sorted(data["head"])[-1]
    say("Latest print: %s   (%d months of history%s)"
        % (last, len(data["head"]), ", live" if live else ", embedded"))

    wb = build(data["head"], data["core"], data["reg"], data["est"])
    out = os.path.abspath(args.out)
    try:
        wb.save(out)
    except PermissionError:
        say("Could not write %s - is it open in Excel?" % out)
        return 1
    say("Wrote %s" % out)
    ...
    say("")
    say("Note: openpyxl writes formulas without cached values, so a few cells may")
    say("look blank until Excel opens the file and calculates. That is normal -")
    say("Excel recalculates on open. If not, press Ctrl+Alt+F9.")
    return 0
