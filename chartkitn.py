...
# ====================================================== HISTORY OVERLAY ====
def history_strip(fig, ax_main, levels, colour="navy", label=None,
                  height_ratio=0.30, bins=44, marks=None):
    """Add a thin panel beneath a chart showing where the rate actually traded.

    The payoff x-axis is TERMINAL spot, and this strip is REALISED spot, so the
    two share units but not meaning. It is context — how much ground the
    currency covers in a year — not a forecast. Label it that way on the chart.

    levels : sequence of daily closes (any length)
    marks  : list of (level, text) to tick, e.g. [(4150, "12m ago")]
    Returns the new axis. Call BEFORE Labeller.resolve() is not required; this
    panel is separate and does not interact with label placement.
    """
    import numpy as np
    # Shrink the main axes and put the strip in the space freed underneath.
    box = ax_main.get_position()
    h = box.height * height_ratio
    ax_main.set_position([box.x0, box.y0 + h, box.width, box.height - h])
    ax = fig.add_axes([box.x0, box.y0, box.width, h * 0.92], sharex=ax_main)

    # The strip takes over the x-axis labelling: two sets of tick labels on the
    # same shared axis is the fastest way to make a chart look broken.
    xlab = ax_main.get_xlabel()
    ax_main.set_xlabel("")
    ax_main.tick_params(labelbottom=False)
    ax.xaxis.set_major_formatter(ax_main.xaxis.get_major_formatter())
    ax.tick_params(labelsize=8.5, colors=C["mut"], length=0, labelbottom=True)
    if xlab:
        ax.set_xlabel(xlab, fontsize=9, color=C["mut"], labelpad=6)

    lv = np.asarray(levels, dtype=float)
    counts, edges = np.histogram(lv, bins=bins)
    centres = (edges[:-1] + edges[1:]) / 2
    dens = counts / counts.max() if counts.max() else counts
    ax.fill_between(centres, 0, dens, color=C[colour], alpha=0.30, lw=0, step="mid")
    ax.plot([lv.min(), lv.max()], [0, 0], color=C[colour], lw=1.8,
            solid_capstyle="butt")

    # Ticks stagger onto two rows when they would otherwise collide, using the
    # same principle as the Labeller: measure, then move.
    xs = ax_main.get_xlim()
    minsep = (xs[1] - xs[0]) * 0.075
    last, row = None, 0
    for lev, txt in sorted(marks or []):
        row = (row + 1) % 2 if (last is not None and lev - last < minsep) else 0
        ax.plot([lev], [0], "|", ms=11, color=C["ink"], mew=1.6)
        ax.text(lev, -0.30 - row * 0.62, txt, ha="center", va="top",
                fontsize=7.5, color=C["mut"])
        last = lev
    ...
    if label:
        ax.text(0, 1.10, label, transform=ax.transAxes, fontsize=8,
                color=C["mut"], va="bottom")
    return ax
...
def save_all(fig, stem, outdir=".", dpi=600, formats=("svg", "png", "pdf")):
    ...
    """
    # tight_layout would undo any manual axes placement (e.g. history_strip),
    # so only apply it when nothing has been positioned by hand.
