    ...
    with open(path, 'r', encoding='utf-8-sig', errors='replace', newline='') as f:
        for row in _csv.reader(f):
            ...
            for cell in row:
                ...
                # Guard: a stray Yes/No column value (e.g. "No") must never be
                # treated as an issuer name -- it would remove EVERY issuer whose
                # name merely contains that text ("no" -> Barrick North, Sinopec,
                # Nomura, ...). Skip 1-char tokens and known flag words.
                if len(norm) < 2 or norm in {'no', 'yes', 'na', 'nan', 'none',
                                            'null', 'true', 'false'}:
                   continue
...
ANALYST_ROSTER = [
    ...
    ('Min Huey Chong',          'Credit Strategist, UBS AG Hong Kong Branch',                                  'Asia corporate bonds'),    
...
MARKET_DATA_NOTE = (
    'Bonds included in this publication may not necessarily be registered '
    'or available in your specific jurisdiction. Market data shown here'
    ' is as of {timestamp}. Prices, yields etc. are indicative values only.'
)
...
REGION_NAME_OVERRIDES = {
    ...
    'Latin America': ('Hyundai Capital America', 'Newmont', 'Barrick', 'Mercado Libre'),
}
...
class GEMDocTemplate(BaseDocTemplate):
    ...
    def _draw_first_page(self, canv, doc):
        ...
        # Top right publication page is provided by Bamboo
    ...
    def _draw_footer(self, canv, doc, include_disclaimer=False, skip_source=False,
        ...
        # Page numbering in lower right performed by Bamboo
...
class GEMPDFBuilder:
    ...
    def _add_guidance_bonds(self):
        ...
        sections = [
            ...
            ('Green, Social, Sustainability',
             ...
             'The designation of these bonds as "sustainable" is based solely on the standards of the Climate '
             'Bonds Initiative ("CBI") as at the date hereof and does not constitute an independent assessment '
             'or guarantee of sustainability by UBS AG or its affiliates (collectively, "UBS"), or any third '
             'party. UBS has relied without independent verification on information from the issuer, and makes '
             'no representation or warranty as to its accuracy or completeness. CBI standards may change, and '
             'continued compliance cannot be assured. This designation does not imply compliance with any '
             'existing law or regulatory framework. Investors should review relevant CBI documentation and '
    ...
    def _render_bond_table(self, rows, show_region_grade_header=False,
        ...
        for row in rows:
            ...
            comment_txt = row['comment']
            if row['green']:   # green / social / sustainability bond
               ref = 'See Page 2 for classification details.'
               comment_txt = f'{comment_txt} {ref}'.strip() if comment_txt else ref
            if comment_txt:
               # Comment row spanning all columns
               data.append([Paragraph(f'<b>Comment:</b> <i>{comment_txt}</i>', s['comment'])] +
    ...
    def _build_change_table(self, title, rows, arrow, full_width=False,
        ...
        show_title=True (default):  render the dark title bar as the table's
                                    first row.
        show_title=False:           omit it — used where the block already
                                    carries its own heading above the table
                                    (the Changes page, where each block is
                                    heading + explanation + table).

        `arrow` is the default direction glyph for every row. A row may
        override it with its own 'arrow' key — needed on the Changes page,
        where a table groups bonds by their NEW view and so can contain both
        ...
        def _arrow_html(glyph):
           if symbol_font:
              return f'<font name="{symbol_font}">{glyph}</font>'
           ascii_map = {'\u2197': '+', '\u2198': '-', '\u2212': '-'}
           return ascii_map.get(glyph, glyph)
        cell_c = ParagraphStyle('chg_cell_c', parent=s['cell'], alignment=TA_CENTER)
        body_rows = []
        for r in rows:
            body_rows.append([
                self._view_cell(r['view_prior']),
                Paragraph(_arrow_html(r.get('arrow') or arrow), arrow_style),
        ...
        # `h` is the index of the column-header row: 1 when the dark title
        # bar is drawn above it, 0 when the caller supplies its own heading.
        data = ([header_bar] if show_title else []) + [col_headers] + body_rows
        h = 1 if show_title else 0
        tbl = Table(data, colWidths=col_widths, repeatRows=h + 1)
        style_cmds = []
        if show_title:
            style_cmds += [
                # Title bar
        ...
        style_cmds += [
            # Column-header row
            ('BACKGROUND',    (0, h), (-1, h), UBS_LIGHT),
            ('TOPPADDING',    (0, h), (-1, h), 3),
            ('BOTTOMPADDING', (0, h), (-1, h), 3),
            # Body rows — tight padding so the narrow view/arrow columns
            # don't end up with negative content width
            ('VALIGN',        (0, 0), (-1, -1), 'MIDDLE'),
            ('LEFTPADDING',   (0, h), (-1, -1), 2),
            ('RIGHTPADDING',  (0, h), (-1, -1), 2),
            ('TOPPADDING',    (0, h + 1), (-1, -1), 2),
            ('BOTTOMPADDING', (0, h + 1), (-1, -1), 2),
            ('LINEBELOW',     (0, h), (-1, -2), 0.25, colors.HexColor('#DDDDDD')),
        ]
        # If empty, span the placeholder across all columns
        if not rows:
            style_cmds.append(('SPAN', (0, h + 1), (-1, h + 1)))
        tbl.setStyle(TableStyle(style_cmds))
        return tbl

    # Explanation blocks for the Changes page. Each entry is
    # (view code as produced by _REC_VIEW, block heading, explanation text).
    # The text is printed directly above the table it describes.
    _CHANGE_BLOCKS = [
        ('attr.', 'Changes to attractive',
         'Drawing on our fundamental analysis of the issuer, as presented in our latest CIO report, we believe this specific bond currently trades at a relatively higher yield than it should compared to others with similar risk profiles, so that we now anticipate it will deliver a higher total return.'),
        ('fair', 'Changes to fair',
         'Drawing on our fundamental analysis of the issuer, as presented in our latest CIO report, we believe this specific bond currently trades at an adequate yield compared to others with similar risk profiles, so that we now anticipate it will deliver a similar total return.'),
        ('exp.', 'Changes to expensive',
         'Drawing on our fundamental analysis of the issuer, as presented in our latest CIO report, we believe this specific bond currently trades at a relatively lower yield than it should compared to others with similar risk profiles, so that we now anticipate it will deliver a lower total return.'),
    ]
    ...
    def _add_changes_section(self):
        ...
        # Group every recommendation change by its NEW view so each block can
        # carry its own explanation immediately above the bonds it describes.
        # Direction is therefore no longer a property of the table — a bond can
        # arrive at 'fair' from either side — so the arrow travels on the row.
        by_new = {code: [] for code, _, _ in self._CHANGE_BLOCKS}
        for key, glyph in (('upgrades', '\u25B2'), ('downgrades', '\u25BC')):
            for row in changes[key]:
                r = dict(row)
                r['arrow'] = glyph
                by_new.setdefault(r['view_new'], []).append(r)
        sort_key = lambda r: ((r['issuer'] or '').lower(), r['isin'])
        for code in by_new:
            by_new[code].sort(key=sort_key)

        head_sty = ParagraphStyle('chg_block_hdr', parent=s['h3'],
                                  spaceBefore=0, spaceAfter=1)
        expl_sty = ParagraphStyle('chg_block_expl', parent=s['body_sm'],
                                  textColor=UBS_MID, spaceAfter=0)

        def block(code, heading, explanation, full_width=False):
            """Heading + its explanation + the matching table, as one unit."""
            return [
                Paragraph(f'<b>{heading}</b>', head_sty),
                Paragraph(explanation, expl_sty),
                Spacer(1, 2*mm),
                self._build_change_table(
                    heading, by_new.get(code, []), '\u25B2',
                    full_width=full_width, show_title=False),
            ]

        blocks = [block(*b) for b in self._CHANGE_BLOCKS]
        grid = Table(
            [[blocks[0], blocks[1]],
             [blocks[2], '']],
            colWidths=[CONTENT_WIDTH / 2, CONTENT_WIDTH / 2])
        ...
        # Layout decision by MEASURED HEIGHT (same guard as the Additions /
        # Deletions page): ReportLab can only split a table BETWEEN rows, so a
        # 2-up grid row that is taller than the frame would die with a
        # LayoutError. Ask how tall it really is and, if it cannot fit, stack
        # the three blocks full-width instead — those split naturally.
        avail_h = CONTENT_HEIGHT - 25*mm   # frame minus the intro text above
        _, grid_h = grid.wrap(CONTENT_WIDTH, avail_h)
        if grid_h > avail_h:
            for i, b in enumerate(self._CHANGE_BLOCKS):
                if i:
                    self.story.append(Spacer(1, 5*mm))
                self.story.extend(block(*b, full_width=True))
        else:
    ...
    def _add_additions_deletions_section(self):
       ...
       if not adds and not dels:
           ...
           # Layout decision by MEASURED HEIGHT, not row count. The 2-up
           # side-by-side grid is a single-row table, and ReportLab can only
           # split tables BETWEEN rows \u2014 a one-row table can never split, so
           # if it is taller than the frame the build dies with a
           # LayoutError ("too large on page ... in frame 'content'").
           # The old row-count threshold (>30) missed weeks where fewer rows
           # still stacked up too tall (wrapped issuer names, etc.).
           # So: build the grid, ASK ReportLab how tall it really is, and
           # fall back to the full-width stacked layout (which splits
           # naturally across pages) whenever it cannot fit.
           avail_h = CONTENT_HEIGHT - 25*mm   # frame minus intro text above
           grid = pair(
               'Additions', adds, '+',
               'Deletions', dels, '\u2212',
           )
