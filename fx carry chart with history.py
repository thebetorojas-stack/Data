...
SPOT      = 3250.0         # current spot, in the units named by QUOTE
...
X_MIN, X_MAX = 2700.0, 4400.0
X_STEP       = 250.0       # gap between x tick labels
Y_MIN, Y_MAX = -26.0, 30.0 # percent
Y_STEP       = 10.0
...
FIG_H_IN = 5.00
DPI_PNG  = 200             # 600 is print grade. 300 is the floor. Beyond 600
...
# 7. Realised-history strip beneath the payoff.
#
#    Pull the series from Bloomberg first. In Excel:
#        =BDH("COP REGN Curncy","PX_LAST",TODAY()-365,TODAY())
#    Save two columns as usdcop_1y.csv with headers  date,px_last
#    (any extra columns are ignored; the date column is not used for the
#     histogram, only for the "12m ago" and "today" markers).
#
#    NOTE ON READING IT: the payoff x-axis is TERMINAL spot twelve months from
#    now; the strip is REALISED spot over the past twelve months. Same units,
#    different meaning. It is there to show how much ground the currency
#    actually covers in a year, so a reader can judge whether the scenario
#    levels are plausible. It is not a forecast and the label says so.
HISTORY_CSV = "usdcop_1y.csv"      # set to None to omit the strip entirely

if HISTORY_CSV:
    import csv
    with open(HISTORY_CSV) as fh:
        hist = [float(r["px_last"]) for r in csv.DictReader(fh)]
    ck.history_strip(
        fig, ax, hist, colour="navy",
        label="Where USDCOP actually traded, last 12 months "
              "(context, not a forecast)",
        marks=[(hist[0], "12m ago"), (min(hist), "low"),
               (max(hist), "high"), (hist[-1], "today")])
    fig._manual_layout = True      # stops save_all re-running tight_layout,
                                   # which would undo the manual panel sizing
